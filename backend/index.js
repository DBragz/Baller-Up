...
db.serialize(() => {
  ...
  // Scores table: single-row store for two team scores
  db.run(
  ...
  db.get('SELECT id FROM scores WHERE id = 1', [], (err, row) => {
    if (err) return; // silently ignore; endpoints will handle errors
    if (!row) {
      db.run('INSERT INTO scores (id, good_score, bad_score) VALUES (1, 0, 0)');
    }
  });
});
...
app.get('/api/scores', (_req, res) => {
  db.get('SELECT good_score AS good, bad_score AS bad FROM scores WHERE id = 1', [], (err, row) => {
    if (err) return res.status(500).json({ error: 'Database error' });
    if (!row) return res.json({ good: 0, bad: 0 });
    res.json({ good: row.good, bad: row.bad });
  });
});

// Update scores (set absolute values)
app.post('/api/scores', (req, res) => {
  const hasGood = typeof req.body?.good === 'number';
  const hasBad = typeof req.body?.bad === 'number';
  if (!hasGood && !hasBad) {
  ...
  if (hasGood) {
    ...
    params.push(Math.max(0, Math.floor(req.body.good)));
  }
  if (hasBad) {
    fields.push('bad_score = ?');
    params.push(Math.max(0, Math.floor(req.body.bad)));
  }

  const sql = `UPDATE scores SET ${fields.join(', ')} WHERE id = 1`;
  db.run(sql, params, function (err) {
    if (err) return res.status(500).json({ error: 'Database error' });
    db.get('SELECT good_score AS good, bad_score AS bad FROM scores WHERE id = 1', [], (gErr, row) => {
      if (gErr) return res.status(500).json({ error: 'Database error' });
      res.json({ good: row.good, bad: row.bad });
    });
  });
